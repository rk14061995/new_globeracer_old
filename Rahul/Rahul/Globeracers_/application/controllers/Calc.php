    ...
    public function getRegUserData(){
        $condition=array("to_show"=>1,"amoun_status"=>"Paid");
         ...
         foreach($regUser as $user){
            ...
            if($user->uId!=""){
                echo '<td> API ID: '.$user->uId.'</td>';
                $connectionType=$user->connection_type;
                echo '<td> '.$connectionType.'</td>';
                if($connectionType=="Strava"){
                    echo '<td> <span style="color:orange">Strava</span> API</td>';
                        //Check For Token Expiry Time
                        
                        if($this->toCheckTokenExpiryTime($user->uId)){
                            echo '<td style="color:green"> Can fetch Strava Details</td>';
                        

                        }else{
                            echo '<td style="color:red"> Cannot fetch Strava Details </td>';
