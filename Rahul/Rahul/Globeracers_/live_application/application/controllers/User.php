<?php
	...
	class User extends MY_Controller
	{
		...
		public function test_one(){
		    //Query For Cumml Team Data By Date
		  //  SELECT sum(`event_details`.`distance`) as dist,`race_registeration`.`reg_id`,`event_details`.*,`team_details`.* FROM `user_details` JOIN `event_details` on `user_details`.`user_id`=`event_details`.`athlete_id` JOIN `race_registeration` ON `user_details`.`id_table`=`race_registeration`.`user_id` JOIN `team_details` ON `race_registeration`.`team_id`=`team_details`.`team_id` WHERE `event_details`.`start_date`='2020-11-18' AND `race_registeration`.`amoun_status`='Paid' GROUP BY `race_registeration`.`team_id` ORDER BY dist DESC
		    $query="SELECT sum(`event_details`.`distance`) as dist,`race_registeration`.`reg_id`,`event_details`.*,`team_details`.* FROM `user_details` JOIN `event_details` on `user_details`.`user_id`=`event_details`.`athlete_id` JOIN `race_registeration` ON `user_details`.`id_table`=`race_registeration`.`user_id` JOIN `team_details` ON `race_registeration`.`team_id`=`team_details`.`team_id` WHERE `event_details`.`start_date`='2020-11-18' AND `race_registeration`.`amoun_status`='Paid' GROUP BY `race_registeration`.`team_id` ORDER BY dist DESC";
    ...
    // 	   print_r($data['teamLeaderBoard']);
    	   ...
    	   //    }
    	   //    if($temp==1){
        // 	           array_push($teamDetails, array("team_id"=>$team->team_id,"distance"=>$distance));
        // 	       }else{
        // 	           //array_push($teamDetails, array("team_id"=>$team->team_id,"distance"=>0));
        // 	           //echo ' Not Exits: '.$team->team_id;
        // 	       }
	   ...
	   ;
		...
		public function fetchDataByFilter(){
		    
		    $date_sel=$this->input->post('date');
		    $filter=$this->input->post('filter_');
		    
		    $query="SELECT SUM(`event_details`.`distance`) as `dist_`,`event_details`.`type`,`event_details`.`start_date`,`team_details`.`team_name`,`user_details`.`user_email`, 
    	   `user_details`.`firstname`,`user_details`.`middle_name` ,`user_details`.`sex`,`user_details`.`lastname` FROM `user_details` JOIN 
    	   `race_registeration` ON `user_details`.`id_table`=`race_registeration`.`user_id`
    	   JOIN `event_details` ON `event_details`.`athlete_id`=`user_details`.`user_id`     JOIN `team_details` ON `team_details`.`team_id`=`race_registeration`.`team_id`                    WHERE
    	   `race_registeration`.`amoun_status`='Paid' and `race_registeration`.`to_show`=1 AND
    	   `event_details`.`start_date`='$date_sel' and `type` LIKE '%".$filter."%' GROUP BY `event_details`.`athlete_id` order by `dist_` desc";
        	$data['filtered_Data']=$this->db->query($query)->result();
        	 if(count($data['filtered_Data'])>0){
        	     die(json_encode(array("code"=>1,"data"=>$data['filtered_Data'])));
        	 }else{
...
// 			die;
		...
		public function myUpComing(){
		  ...
		  // echo 'id: '.$user_id;  
		     if($atlete_id!=""){
		        ...
		        $data['distanceSum']=$this->db->query('SELECT *,SUM(distance) as totalDistance FROM event_details where athlete_id='.$atlete_id.' AND start_date >="2020-11-14"')->result();
		     }else{
		         $data['distanceSum']=0;
		     }
		...
		public function myEventActivities(){
		    ...
		    $data['activities']=$this->db->where($condition)->get('event_details')->result();
...
// 			}
			...
			$condition=array(
								"event_details.athlete_id"=>$user_id,
									"event_details.start_date >="=>'2020-11-14',
								// "userevents.event_start_date>="=>date('Y-m-d')
							);
			$data['events']=$this->db->where($condition)->order_by('start_date','desc')->get('event_details')->result();
