...
class API extends CI_Controller {
    ...
    public function log_Out(){
	    ...
	    if($this->session->unset_userdata('userData')){
	        redirect('API/viewEventDescription/10');
	    }else{
	        redirect('API/viewEventDescription/10');
	    }
    }
...
// 			echo 'Refresh Token: '.$refresh_token;
	...
	public function payumoney($reg_id){
	   // die('Server is Down. Try Again Later');
		    $ip = $_SERVER['REMOTE_ADDR'];
        // $ip ="77.99.179.98";
        $details = json_decode(file_get_contents("http://ipinfo.io/{$ip}/json"));
        $condition=array(
...
// 	public function payumoney_($reg_id){
// 		    $ip = $_SERVER['REMOTE_ADDR'];
//         // $ip ="77.99.179.98";
//         $details = json_decode(file_get_contents("http://ipinfo.io/{$ip}/json"));
//         $condition=array(
...
// 	public function viewEventDescription_($eventId){
// 			$condition=array(
...
// 			$this->load->view('layout/footerUser');
    ...
    public function googleLogin(){
        ...
        $count=$this->db->where($conditiondata)->get('user_details')->result();
        if(count($count)==0)
        {
            ...
            $userData=array(
                ...
                {
                    // echo ' Data Inserted';
                    $uData=$this->db->where($conditiondata)->get('user_details')->result();
                    // userData?
                    $this->session->set_userdata('userData',serialize($uData));
                    die(json_encode(array("code"=>1,"msg"=>"Data Inserted successfull", "type"=>"Normal","data"=>$uData)));
                }
                else
                {
                    die(json_encode(array("code"=>0,"msg"=>"Cannot Register")));
                }
            
        }
        else
        {
              $this->session->set_userdata('userData',serialize($count));
            die(json_encode(array("code"=>1,"msg"=>"Data Inserted","data"=>$count)));
        }
